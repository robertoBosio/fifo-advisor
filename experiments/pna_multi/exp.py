...
# Locate the synthesized PNA design generated by `experiments/single_synth/synth.py`.
# Prefer the shared pre-synth directory; fall back to the local cache under
# `experiments/single_synth/test_cases/` if the design has not been copied yet.
DIR_SINGLE_SYNTH = DIR_CURRENT.parent / "single_synth" / "test_cases"
candidate_dirs = [DIR_PRE_SYNTH / "pna", DIR_SINGLE_SYNTH / "pna"]

designs_all_filtered: list[TestCase] = []
for candidate in candidate_dirs:
    if candidate.exists() and candidate.is_dir():
        designs_all_filtered.append(
            TestCase.from_dir(candidate, candidate.name.split("__")[0])
        )
        break

if not designs_all_filtered:
    raise FileNotFoundError(
        "PNA test case not found. Run `python experiments/single_synth/synth.py` "
        "or copy the synthesized project into DIR_PRE_SYNTH before running this "
        "experiment."
    )
