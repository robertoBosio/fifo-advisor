...
template <typename TInputWord, typename TOutput, typename TOutputWord,
          typename TWindow, size_t DATA_PER_WORD, size_t DATA_TO_SHIFT,
          size_t TIMES, size_t WORDS, size_t ARRAY_PAR, size_t WORD_PAR>
...
private:
  ...
  static void pipeline_body(hls::stream<TWindow> o_data[1],
...
#pragma HLS inline
    ...
    for (size_t i_fhw = 0; i_fhw < ARRAY_PAR; i_fhw++) {
      ...
      window[i_fhw] = out_word;
    }
    o_data[0].write(window);
  }
...
public : StreamingMemory() = default;
  ...
  struct StepState {
    ...
    PipelineDelayBuffer<TWindow> delayed_output;
    ...
    void init(size_t depth) {
      if (initialized)
        return;
      delayed_output = PipelineDelayBuffer<TWindow>(depth);
      this->depth = depth;
...
#pragma HLS pipeline II = 1
  ...
  ActorStatus step(hls::stream<TWindow> o_data[1]) {
    static TOutput mem[CH_GROUPS][WORD_PAR][ARRAY_PAR];
    ...
    if (st.depth == 1) {
      ...
      // instead.
      if (st.delayed_output.peek() && o_data[0].size() >= STREAM_DEPTH) {
        firing_condition = false;
      }
    }

    if (firing_condition) {
      hls::stream<TWindow> instant_output_window[1];
      StreamingMemory::pipeline_body(instant_output_window,
                                     mem[st.i_ch_groups]);
      st.i_ch_groups++;
      ...
      if (!instant_output_window[0].empty()) {
        st.delayed_output.push(instant_output_window[0].read(), true);
      } else {
        // If the output stream is empty, push a placeholder.
        st.delayed_output.push(TWindow(), false);
      }
