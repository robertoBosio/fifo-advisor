...
template <typename TInputWindow, typename TInputWord, typename TInput,
          typename TWeightWindow, typename TWeightWord, typename TWeight,
          typename TBiasWord, typename TBias, typename TOutputWord,
          typename TOutput, typename TSum, typename TPartialSum,
...
#pragma HLS ARRAY_PARTITION variable = input_data dim = 0
    ...
    for (size_t i_hw = 0; i_hw < OUT_HEIGHT * OUT_WIDTH / W_PAR; i_hw++) {
      for (size_t i_ich = 0, weight_group = 0; i_ich < IN_CH;
           i_ich += IN_CH_PAR) {
      STREAMINGCONV_RUN_LOOP:
        for (size_t i_och = 0, bias_group = 0; i_och < OUT_CH;
             i_och += OUT_CH_PAR, bias_group++, weight_group++) {
#pragma HLS pipeline II = 1
          StreamingConv::pipeline_body(
              i_data, i_weights[weight_group], i_biases[bias_group], o_data,
              input_data, acc_buff[i_och / OUT_CH_PAR], i_ich, i_och);
        }
      }
    }
  }

  ActorStatus step(hls::stream<TInputWindow> i_data[1],
                   hls::stream<TWeightWindow> i_weights[1],
                   hls::stream<TBiasWord> i_biases[1],
                   hls::stream<TOutputWord> o_data[W_PAR]) {
  ...
  ActorStatus
  step(hls::stream<TInputWindow> i_data[1],
       TWeight i_weights[CH_GROUPS][OUT_CH_PAR * IN_CH_PAR][FH * FW],
       TBias i_biases[OUT_CH / OUT_CH_PAR][OUT_CH_PAR][1],
       hls::stream<TOutputWord> o_data[W_PAR]) {
    // Get the state for this instance.
...
private:
  static void pipeline_body(hls::stream<TInputWindow> i_data[1],
                            hls::stream<TWeightWindow> i_weights[1],
                            hls::stream<TBiasWord> i_biases[1],
                            hls::stream<TOutputWord> o_data[W_PAR],
                            TInputWord input_data[FH][FW_EXPAND],
...
#pragma HLS ARRAY_PARTITION variable = acc_buff_par_local dim = 0
    ...
    if (i_och == 0) {
      TInputWindow input_window = i_data[0].read();
      for (size_t fh = 0; fh < FH; fh++) {
        for (size_t fw = 0; fw < FW_EXPAND; fw++) {
          input_data[fh][fw] = input_window[fh * FW_EXPAND + fw];
        }
      }
    }

    // Read the weight data for the current filter.
    TWeightWindow weight_window = i_weights[0].read();
    for (size_t fh = 0; fh < FH; fh++) {
      for (size_t fw = 0; fw < FW; fw++) {
        weight_data[fh][fw] = weight_window[fh * FW + fw];
      }
    }
    ...
    for (size_t i_w_par = 0; i_w_par < W_PAR; i_w_par++) {
      for (size_t i_och_par = 0; i_och_par < OUT_CH_PAR; i_och_par++) {
        ...
        // finalize the output.
        if (i_ich == IN_CH - IN_CH_PAR) {
          TSum wide_acc = acc_buff_par[acc_index] + bias_data[0][i_och_par];
