...
template <typename TWord, typename TWindow, typename TData, size_t IN_HEIGHT,
          size_t IN_WIDTH, size_t IN_CH, size_t FH, size_t FW, size_t STRIDE_H,
          size_t STRIDE_W, size_t DILATION_H, size_t DILATION_W, size_t PAD_T,
          size_t PAD_L, size_t PAD_B, size_t PAD_R, size_t W_PAR, size_t CH_PAR,
          int PAD_VALUE = 0>
class StreamingPad {
...
#pragma HLS pipeline II = 1
  ...
  ActorStatus step(hls::stream<TWord> i_data[FH * FW_EXPAND],
    ...
    if (st.depth == 1) {
      ...
      if (st.delayed_output.peek() && o_data[0].size() >= 2) {
        firing_condition = false;
      }
    }

    if (firing_condition) {
      hls::stream<TWindow> instant_o_data[1];
      ...
      st.delayed_output.push(instant_o_data[0].read(), true);
...
#pragma HLS inline
    ...
    for (size_t i_fh = 0; i_fh < FH; i_fh++) {
      for (size_t i_fw = 0; i_fw < FW_EXPAND; i_fw++) {
        ...
        out_window[i_fh * FW_EXPAND + i_fw] = in_word;
      }
    }
    o_data[0].write(out_window);
  }
};